...
# Sample data creation if files don't exist
def create_sample_data():
    # Sample symptoms and conditions
    symptoms_conditions = {
    ...
    # Sample conditions and treatments
    conditions_treatments = {
        'Flu': ['Rest', 'Hydration', 'Over-the-counter medication'],
        'COVID-19': ['Isolation', 'Rest', 'Consult doctor'],
        'Common Cold': ['Rest', 'Hydration', 'Vitamin C'],
        'Migraine': ['Dark room', 'Pain medication', 'Rest']
    }
    
    # Create CSV files if they don't exist
    if not os.path.exists('data/symptoms_conditions1.csv'):
        with open('data/symptoms_conditions1.csv', 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Symptom', 'Condition'])
            for symptom, conditions in symptoms_conditions.items():
                for condition in conditions:
                    writer.writerow([symptom, condition])
    
    if not os.path.exists('data/conditions_treatments.csv'):
        with open('data/conditions_treatments.csv', 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Condition', 'Treatment'])
            for condition, treatments in conditions_treatments.items():
    ...
    if not os.path.exists('data/doctors.csv'):
        with open('data/doctors.csv', 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Name', 'Specialty', 'Available'])
            writer.writerow(['Dr. Smith', 'General Medicine', 'Yes'])
            writer.writerow(['Dr. Johnson', 'Pediatrics', 'Yes'])
    
    if not os.path.exists('data/appointments.csv'):
        with open('data/appointments.csv', 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['ID', 'Name', 'Time', 'Date', 'Illness', 'Doctor', 'Title', 'Description'])
...
def find_closest_slot(preferred_time, available_slots):
    try:
        ...
        print(f"Error finding closest slot: {e}")
        return None
...
def match_symptoms(user_input):
    try:
        ...
        for symptom in symptoms_conditions_dict.keys():
            if any(token in preprocess_text(symptom.lower()) for token in preprocessed_input):
...
def chatbot():
    try:
        ...
        if not state['name']:
            return jsonify({'response': "I didn't catch your name. What should I call you?"})
